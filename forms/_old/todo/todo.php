...
function ajax__todo_get() {
  $list = wbItemList("todo");
  $json=[];
...
function _todoBeforeItemSave(&$Item) {
  $Item = (object)$Item;
  if ($Item->time > "") {
      $Item->time = date("Y-m-d H:i:s",strtotime($Item->time));
  } else {
    $Item->time = date("Y-m-d H:i:s",strtotime("+24 hours"));
  }
...
function _todoAfterItemRead($Item) {
