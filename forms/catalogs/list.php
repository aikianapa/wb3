    ...
    <div id="{{_form}}List" wb-off>
            ...
            <div class="collapse navbar-collapse order-2" id="navbarSupportedContent">
                <form class="form-inline mg-t-10 mg-lg-0  ml-auto" onsubmit="return false;">
                    <div class="form-group">
                        <input class="form-control mg-r-10 col-auto rounded-10" type="search" placeholder="Поиск..." aria-label="Поиск..." on-keyup="find">
                    </div>
                    <a href="#" data-ajax="{'url':'/cms/ajax/form/catalogs/edit/_new','html':'modals'}" class="ml-2 btn btn-primary">
        ...
        <ul class="list-group">
            ...
            <li class="list-group-item d-flex align-items-center" data-id="{{.id}}">
                <div>
                    <a href="javascript:" data-ajax="{'url':'/cms/ajax/form/{{_form}}/edit/{{_id}}','html':'modals'}" class="tx-13 tx-inverse tx-semibold mg-b-0">{{_id}}</a>
                    <span class="d-block tx-11 text-muted">{{name}}&nbsp;</span>
                </div>

                <div class="custom-control custom-switch pos-absolute r-80">
                    {{#if active=='on' }}
                    <input type="checkbox" class="custom-control-input" name="active" checked id="{{_form}}SwitchItemActive{{ @index }}" onchange="wbapp.save($(this),{'table':'{{_form}}','id':'{{_id}}','field':'active'})"> {{/if}} {{#if active !='on' }}
                    <input type="checkbox" class="custom-control-input" name="active" id="{{_form}}SwitchItemActive{{ @index }}" onchange="wbapp.save($(this),{'table':'{{_form}}','id':'{{_id}}','field':'active'})"> {{/if}}
                ...
                <div class="pos-absolute r-10 p-0 m-0" style="line-height: normal;">
                    <a href="javascript:" data-ajax="{'url':'/cms/ajax/form/{{_form}}/edit/{{_id}}','html':'modals'}" class=" d-inline">
        ...
        var list = new Ractive({
            ...
            on: {
                ...
                find(ev) {
                    ...
                    let datalist = $('#{{_form}}List .list-group')
                    if (find == '') {
                        $(datalist).find('.list-group-item').removeClass('d-none').addClass('d-flex')
                        return
                    } else {
                        $(datalist).find('.list-group-item').addClass('d-none').removeClass('d-flex')
                    }
                    $(datalist).find('.list-group-item').each(function() {
