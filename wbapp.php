...
class wbDom extends DomQuery
{
    ...
    {
        $res = true;
        if (!$this->params("where")) return $res;
        if ($this->params("where") == "") return $res;
    ...
    {
        $attrs = [];
        foreach($this->attributes as $k => $v) $attrs[] = $v->name;
        return in_array($attr,$attrs);
    }

    public function head($head = null) {
        if (!isset($this->head)) return false;
        if ($head == null) return $this->head;
        //$this->head->html($head);
    }
    ...
    public function fetchNode() {
      ...
      if (isset($this->fetched)) return;
      $this->fetchParams();
      if ($this->role AND ($this->func OR $this->funca)) $this->fetchFunc();
      $this->fetch();
      $this->fetched = true;
      return $this;
    }
    ...
    public function fetchStrict() {
      if ($this->tagName == "template") $this->strict = true;
        ...
        function params($name = null) {
            $res = null;
            if ($name == null) {
                if (isset($this->params)) $res = $this->params;
            } else {
                if (isset($this->params->$name)) $res = $this->params->$name;
            }
            return $res;
        }
        ...
        public function fetchParams()
        {
            ...
            $this->atrs = (object)[];
            $params = [];
            if (substr($this->tagName,0,3) == "wb-") $this->role = substr($this->tagName,3);
            $attrs = $this->attributes();
            if (count($attrs)) {
            ...
            if ($this->role) {
                ...
                if (is_file($file)) {
                  require_once $file;
                  if (class_exists($func)) $this->func = $func;
                } else {
                  unset($this->role);
                }
            }
        ...
        public function copy($parent) {
          if (isset($parent->head)) $this->head = $parent->head;
          else $this->head = false;
          if (isset($parent->strict)) $this->strict = $parent->strict;
...
class wbApp
{
    ...
    function driver() {
        ...
        } else if (is_file($this->route->path_engine.$path)) {
    ...
    function controller()
    {
        if ($this->route->controller) {
            ...
            if (is_file($this->route->path_app . $path)) require_once $this->route->path_app . $path;
            else if (is_file($this->route->path_engine.$path)) require_once $this->route->path_engine.$path;
            $class = "ctrl".ucfirst($this->route->controller);
            if (!class_exists($class)) echo "Controller not found: {$this->route->controller}";
            else new $class($this);
            die;
        }
    }
