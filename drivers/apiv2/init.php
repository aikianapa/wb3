...
class apiv2Drv
{
    ...
    public function __construct(&$app)
    {
        $this->app = &$app;
        $this->driver = 'apiv2';
        $this->ini = $app->settings->driver_options["apiv2"];
    ...
    public function connect() {
        ...
        $this->api = $this->ini["api"];
        $this->token = $this->app->vars('_sess.'.$this->driver.'.token');
        if (isset($this->ini["token"])) {
            $this->token = "?__token=".$this->ini["token"];
        } else if ($this->token == '') {
            $l = $this->ini['login'];
            $p = $this->ini['password'];
            $result = $this->app->authPostContents($this->api."/login", ['login'=>$l,'password'=>$p]);
            $res = $this->app->dot(json_decode($result,true));
            if ($res->get('error')) {
                echo $result;
                exit;
            } else {
                $this->token = $res->get('token');
                $this->app->vars('_sess.'.$this->driver.'.token', $res->get('token'));
            }
        }
        return $this;
    }
    ...
    public function itemList($form = 'pages', $options = [])
    {
        ...
        $cid = md5($form . json_encode($options) . $_SESSION['lang']);
        if (isset($_ENV['cache'][$cid])) {
            $list = $_ENV['cache'][$cid];
        } else {
            $query = $this->api."/list/{$form}";
            $options['__token'] = $this->token;
            $list = $this->app->authPostContents($query, $options);
...
*/
    ...
    public function itemRead($form = null, $id = null)
    {
            ...
            $query = $this->api."/read/{$form}/$id";
            $item = $this->app->authPostContents($query, ['__token'=>$this->token]);
