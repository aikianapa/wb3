        ...
        var ypbrBlocks = new Ractive({
            ...
            on: {
                complete() {
                    ...
                    $(ypbrBlocks.target).nestable({
                        ...
                        beforeDragStop: function(l, e, p) {
                            let data = {};
                            setTimeout(() => {
                                $(ypbrBlocks.target).find('.dd-item').each(function() {
                                    let id = $(this).attr('data-id');
                                    data[id] = ypbrBlocks.get('blocks.' + id);
                                });
                    ...
                    $(ypbrBlocks.target).find('li.dd-item:first img.edit').trigger('click');
                },
                store() {
                ...
                edit(ev) {
                    ...
                    ypbrBlocks.blockEdit(id);
                },
                copy(ev) {
                    let id = $(ev.node).parents('.dd-item').attr('data-id');
                    let item = ypbrBlocks.get('blocks.'+id);
                    id = wbapp.newId();
                    item.header += ' (копия)';
                    item.id = id;
                    ypbrBlocks.set('blocks.'+id,item);
                    ypbrBlocks.fire('store')
                },
                remove(ev) {
                    let that = ev.node;
                    let id = $(that).parents('.dd-item').attr('data-id');
                    if (id > '') {
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0 pb-5 scroll-y">
                    <div class="list-group">
                        {{#each blocks}}
                        <a class="list-group-item d-flex align-items-center" href="javascript:void(0)"
        ...
        wbapp.ajax({
                ...
                on: {
                    viewPreview(ev, view) {
                        ...
                        if (view && !$('#yongerBlockPrewiew').is(':visible')) {
                            ...
                            $('#yongerBlockPrewiew iframe').attr('src',
                                `/module/yonger/blockpreview?block=${data.name}`)
            ...
            $blockslist
                ...
                .delegate('.list-group-item', wbapp.evClick, function() {
                    ...
                    ypbrBlocks.fire('store')
                    setTimeout(() => {
                        $(ypbrBlocks.target).find('li.dd-item:last img.edit').trigger('click');
                    }, 100);
