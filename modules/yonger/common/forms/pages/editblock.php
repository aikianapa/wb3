<html>
<div id="yongerBlockEditor" class="modal effect-scale removable" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog">
            ...
            <div class="modal-header">
                <div class="modal-title ">
                    <ul class="nav nav-tabs" id="yongerBlockEditorTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" data-toggle="tab" href="#yongerBlockEditorView" role="tab">{{_lang.view}}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" data-toggle="tab" href="#yongerBlockEditorEdit" role="tab">{{_lang.form}}</a>
                        </li>
                    </ul>
                </div>
                <div>
...
<script>
    $('#yongerBlockEditor a[data-toggle="tab"]').on('shown.bs.tab', function(ev) {
        let tab = ev.currentTarget.getAttribute('href');
        let editor = $(tab).find('textarea.codemirror').data('editor');
        editor.refresh();
    })
