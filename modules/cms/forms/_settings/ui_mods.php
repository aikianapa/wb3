...
<div>
    <wb-foreach wb-call="wbListModules()">
        <button type="button" class="btn btn-sm btn-light d-inline m-1" wb-if='"{{sett}}" > ""' data-ajax="{'url':'/module/{{id}}/_settings/','html':'#editSettingsForm'}">
            {{id}}
        </button>
    </wb-foreach>
</div>
<template id='modSettingsWrapper'>
    <div>
        <nav class="nav navbar navbar-expand-md col">
            <wb-lang>
            [en] header = "Module settings"
            [ru] header = "Настройки модуля"
            </wb-lang>
        ...
        <wb-data wb="{'table':'_settings','item':'settings','field':'modules.{{module}}'}">
            <form class="col">
                ...
                <button type="button" class="btn btn-primary tx-13 r-0" wb-save="{'table':'_settings','item':'settings','field':'modules.{{_parent.module}}'}">
